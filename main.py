...
def enforce_terminology(text: str, rules: List[Dict[str, Any]]) -> str:
    ...
    for rule in rules:
        if rule["is_regex"]:
            ...
            # ⚡ BOLT OPTIMIZATION: Check if substring exists before calling .replace() to avoid string allocation overhead
...
def extract_metadata_from_filename(filename: str) -> Dict[str, str]:
    ...
    # ⚡ BOLT OPTIMIZATION: Use conditional .endswith() slicing instead of chained .replace() calls
...
def process_file(client: genai.Client, file_path: str, output_dir: str, rules: Dict[str, Any]):
    """Processes a single .txt file and saves the .md result."""
    filename = os.path.basename(file_path)
    
    # Check if output already exists (Idempotency)
    name, _ = os.path.splitext(filename)
    output_filename = f"{name}_rag_optimized.txt"
...
def main():
    ...
    parser.add_argument("--dir", default=".", help="Diretório contendo os arquivos .txt")
    ...
    files = [f for f in os.listdir(args.dir) if f.endswith('.txt') and "Transcrição" in f and "_rag_optimized" not in f]
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Exceção não tratada no worker para {filename}: {e}")
                
    end_time = time.time()
    logger.info(f"Processamento de lote concluído em {end_time - start_time:.2f} segundos.")
