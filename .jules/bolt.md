...
## 2026-07-07 - [Optimize structured block parsing with str.find]
**Learning:** When extracting specific data from a delimited block of text (like finding text after a `-->` marker in SRT blocks), splitting the block into lines and iterating through them creates unnecessary array allocations and string operations.
**Action:** Instead of `text.split('\n')` and iterating, use `str.find('marker')` to locate the target, followed by another `str.find('\n', start_index)` to locate the start of the relevant text, and use string slicing `block[start_index:]`. This avoids creating intermediate lists of lines and is significantly faster.
