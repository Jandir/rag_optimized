...
## 2026-06-25 - [Optimize SRT Block Parsing with Native String Search]
**Learning:** For extracting specific portions of structured text blocks (like the text following a '-->' marker in SRT files), splitting the block into lines and iterating through them to find the marker is noticeably slower (~25% overhead).
**Action:** When locating specific markers within predictably structured blocks, use fast native string searches (`str.find`) and slicing (`text[index+1:]`) instead of splitting the block into lines and iterating. This avoids unnecessary memory allocations for intermediate list objects and is computationally faster.
