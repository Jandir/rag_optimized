...
## 2024-07-31 - [Optimize SRT parsing with native string find]
**Learning:** For locating specific markers (like '-->') within blocks of text, using `str.find` and slicing is approximately 2.5x to 3x faster than splitting the block into lines and iterating line-by-line. This avoids creating many small, temporary string objects (lines).
**Action:** When parsing structures like SRT blocks, use fast native string searches (`str.find`) and slicing instead of splitting blocks into lines, iterating, and joining them back.
