...
## 2026-07-27 - [Fast String Search for Block Parsing]
**Learning:** When parsing SRT blocks, splitting each block by newlines and iterating is slower than just using `.find()` and slicing the block string.
**Action:** Use fast native string methods like `.find('-->')` and slicing instead of allocating new lists by splitting blocks into individual lines.
