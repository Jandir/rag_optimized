...
## 2026-08-01 - [Avoid loop-appends for SRT block parsing]
**Learning:** Parsing blocks of strings line-by-line using loops and `str.split('\n')` creates significant overhead in highly repetitive text like SRT files. Replacing it with `str.find('-->')` and slicing natively avoids memory overhead.
**Action:** Always prefer native `str.find` and index slicing over `str.split` with inner loops for finding data within predictable text blocks.
