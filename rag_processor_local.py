...
def _parse_srt_blocks(content_str: str) -> List[str]:
    ...
    for block_str in raw_blocks_list:
        lines_list: List[str] = [line_str.strip() for line_str in block_str.split('\n') if line_str.strip()]
        timestamp_idx_int: int = -1
        for i_int, line_str in enumerate(lines_list):
            if '-->' in line_str:
                timestamp_idx_int = i_int
                break

        if timestamp_idx_int != -1 and len(lines_list) > timestamp_idx_int + 1:
            text_block_str: str = '\n'.join(lines_list[timestamp_idx_int+1:]).strip()
