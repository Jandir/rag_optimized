...
def _parse_srt_blocks(content_str: str) -> List[str]:
    ...
    # ⚡ BOLT OPTIMIZATION: Use fast native string find and slicing instead of splitting into lines
    blocks_list: List[str] = []
    for block_str in content_str.split('\n\n'):
        arrow_idx_int: int = block_str.find('-->')
        if arrow_idx_int != -1:
            end_of_line_idx_int: int = block_str.find('\n', arrow_idx_int)
            if end_of_line_idx_int != -1:
                text_block_str: str = block_str[end_of_line_idx_int + 1:].strip()
