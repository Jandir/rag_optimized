...
def _parse_srt_blocks(content_str: str) -> List[str]:
    ...
    for match_obj in SRT_BLOCK_PATTERN.finditer(content_str):
        ...
        # ⚡ BOLT OPTIMIZATION: Fast-path 'in' guard bypasses expensive regex execution when tag isn't present
...
def clean_srt_content(content_str: str) -> str:
    ...
    """
    # ⚡ BOLT OPTIMIZATION: Added fast-path 'in' guard before string replace
    if '\r\n' in content_str:
...
def enforce_terminology(text_str: str, rules_list: List[Dict[str, Any]]) -> str:
    ...
    for rule_dict in rules_list:
        if rule_dict["is_regex"]:
            ...
            # ⚡ BOLT OPTIMIZATION: Added fast-path 'in' guard before .replace() overhead
...
def extract_metadata_from_filename(filename_str: str) -> Dict[str, str]:
    ...
    # ⚡ BOLT OPTIMIZATION: Avoid magic numbers in slicing length calculations
