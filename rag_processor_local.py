...
def _parse_srt_blocks(content_str: str) -> List[str]:
    ...
    # ⚡ BOLT OPTIMIZATION: Use native string search and slicing to avoid list creation and loop overhead
    blocks_list: List[str] = []
    for block_str in content_str.split('\n\n'):
        arrow_idx_int: int = block_str.find('-->')
        if arrow_idx_int != -1:
            next_newline_idx_int: int = block_str.find('\n', arrow_idx_int)
            if next_newline_idx_int != -1:
                text_block_str: str = block_str[next_newline_idx_int + 1:].strip()
