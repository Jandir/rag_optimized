...
def _deduplicate_srt_lines(blocks_list: List[str]) -> List[str]:
    ...
    for curr_text_str in blocks_list[1:]:
        ...
        if prev_lines_list is None:
            prev_lines_list = [line.strip() for line in prev_text_str.splitlines() if line.strip()]

        curr_lines_list: List[str] = [line.strip() for line in curr_text_str.splitlines() if line.strip()]
        start_idx_int: int = 0
...
class HeuristicProcessor:
    ...
    def clean_filler_words(self, text_str: str) -> str:
        ...
        return FILLERS_PATTERN.sub('', text_str).replace('  ', ' ').strip()
