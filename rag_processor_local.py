...
class HeuristicProcessor:
    ...
    def _generate_markdown_output(self, meta_dict: Dict[str, str], entities_list: List[str], keywords_list: List[str], sections_list: List[str]) -> str:
        ...
        # ⚡ BOLT OPTIMIZATION: Use list append and ''.join() instead of string concatenation (+=)
        # to avoid O(n²) memory reallocation overhead when building large strings dynamically.
        parts_list: List[str] = []
        parts_list.append("---\n")
